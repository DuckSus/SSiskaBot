...
responses = {
    ...
    2: [
        ...
        "Магазинные лифчики хоть как-то сидят. 🛍️",
...
async def inline_breast_size(query: InlineQuery):
   ...
   # Варианты начала фразы
    intro_phrases = [
        ...
        "🍒 Кажется у тебя {size} размер!",
        "💀 У твоего нерожденного брата был бы {size} размер!",
        "🍒 У твоей знакомой подруги троюдной сестры {size} размер",
        "🍒 У твоей маминой подруги {size} размер!",
    ]
...
# Проверяем шанс выпадения редкого интро (пенис-интро)
if random.random() < 0.01:  # 1% шанс на интро про пенис
    intro = random.choice(rare_intro_phrases).format(size=size)
    text = f"{intro}\n\n💡СиськаФакт - {fact}"  # Без описания
else:
    intro = random.choice(intro_phrases).format(size=size)
    text = f"{intro}\n{description}\n\n💡СиськаФакт - {fact}"  # Обычный вариант
